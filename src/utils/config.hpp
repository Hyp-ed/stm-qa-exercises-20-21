 ...
 * First, a list of modules is defined. The list takes and argument V, which
 * will be another macro. As part of the main list definition, relevant
 * information can be passed to the V argument. This can be several values for
 * each entry, e.g. variable name, type, access level,... In this case, it is
 * just the name of the module.
 *
 * Later, we define the macro V to perform the code generation for us.
 * For example, it is used right here to define enums for the modules, see
 * CREATE_ENUM. It is also used in the Config class declaration to declare the
 * parsing method for each module, see DECLARE_PARSE. Later, in .cpp file, it is
 * used to define module map entries, see MAP_ENTRY.
 */
...
class Config {
 ...
 public:
  ...
  struct InterfaceFactory {
    // Module used in this context refers to the namespace containing the
    // interface.
